  ...
  <div>
    ...
    <div class="flex gap-4 text-xs mb-3 justify-center">
      <span class="flex items-center gap-1"
        ><span class="w-3 h-3 rounded bg-(--color-success)/15" /> 已熟悉</span
      >
      <span class="flex items-center gap-1"
        ><span class="w-3 h-3 rounded bg-(--color-warning)/15" /> 需加強</span
      >
      <span class="flex items-center gap-1"
        ><span class="w-3 h-3 rounded bg-gray-50 border border-gray-200" />
        未練習</span
      >
    </div>
        ...
        <thead>
          <tr>
            ...
            <th
              ...
              :key="col"
              class="p-1 text-xs text-(--color-text-secondary)"
            >
              {{ col.charAt(0) }}
            </th>
          </tr>
          ...
          >
            ...
            >
              <div
                v-if="
                  getEntry(
                    showType === 'hiragana' ? hiragana : katakana,
                    row,
                    col,
                  )
                "
